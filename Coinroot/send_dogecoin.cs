...
namespace Coinroot
{
    public partial class send_dogecoin : Form
    {
        public send_dogecoin()
        {
            InitializeComponent();
        ...
        private void send_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var a = DogecoinService.ValidateAddress(address_textBox.Text);
                Console.WriteLine(a);
                if (a.IsValid == true)
                {
                    if (amount_numericUpDown.Value <= DogecoinService.GetBalance())
                    {
                        var b = Convert.ToDecimal(amount_numericUpDown.Value);
                        DogecoinService.SendToAddress(address_textBox.Text, b, null, null);
